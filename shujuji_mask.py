...
class DatasetGeneratorPage(QWidget):
    """数据集生成页面（带自动文件夹分类功能）"""
    ...
    def __init__(self):
        ...
        self.processed_image = None  # 带污渍图像
        self.processed_mask = None  # 污渍 Mask 图像
        self.image_paths = []  # 批量处理的图像路径列表
        self.output_dir = ""  # 输出目录
        self.stained_dir = ""  # 处理后图片保存目录
        self.mask_dir = ""  # 掩码图片保存目录
        self.processing_thread = None  # 处理线程
        self.is_processing = False  # 是否正在处理
        self.is_mask_preview = False  # 是否显示 Mask 预览
    ...
    def init_ui(self):
        ...
        self.basic_stain_count.setValue(5)
        ...
        self.basic_max_ratio.setValue(5)
        ...
        self.basic_min_ratio.setValue(1)
        ...
        self.basic_edge_bias.setValue(30)
        ...
        self.advanced_stain_count.setValue(3)
        ...
        self.advanced_max_ratio.setValue(15)
        ...
        self.advanced_min_ratio.setValue(3)
        ...
        self.advanced_edge_bias.setValue(60)
        ...
        self.advanced_complexity.setValue(5)
        ...
        # 新增提示：说明会自动创建子文件夹
        subfolder_note = QLabel("提示：将自动创建 'stained_images' 和 'masks' 子文件夹")
        subfolder_note.setFont(QFont("SimHei", 8))
        subfolder_note.setStyleSheet("color: #64748b; italic;")
        output_layout.addWidget(subfolder_note)
        ...
        preview_label = QLabel("图像预览（原始 vs 带污渍/Mask）")
        ...
        self.toggle_preview_btn = QPushButton("切换：显示带污渍图")
        self.toggle_preview_btn.setFont(QFont("SimHei", 10))
        ...
        processed_title = QLabel("带污渍图像/Mask")
    ...
    def apply_basic_stains(self, image):
        ...
        return np.clip(stained_image, 0, 255).astype(np.uint8), combined_mask
    ...
    def create_advanced_stain_params(self, height, width):
        ...
        for _ in range(num_stains):
            ...
            if stain_type == 'crack':
                ...
                color = (random.randint(40, 70), random.randint(30, 60), random.randint(25, 55))
                alpha = random.uniform(0.7, 0.9)
            elif stain_type == 'erosion':
                texture_intensity = random.uniform(0.7, 0.9)
                color = (random.randint(80, 110), random.randint(70, 100), random.randint(60, 90))
                alpha = random.uniform(0.3, 0.6)
            elif stain_type == 'sand_wear':
                texture_intensity = random.uniform(0.6, 0.8)
                color = (random.randint(100, 130), random.randint(90, 120), random.randint(80, 110))
                alpha = random.uniform(0.3, 0.6)
            else:
                texture_intensity = random.uniform(0.4, 0.7)
                color = (random.randint(70, 100), random.randint(60, 90), random.randint(50, 80))
                alpha = random.uniform(0.3, 0.6)
    ...
    def toggle_preview(self):
        ...
        if self.is_mask_preview:
            self.toggle_preview_btn.setText("切换：显示Mask图")
            if self.processed_mask is not None:
                mask_rgb = cv2.cvtColor(self.processed_mask, cv2.COLOR_GRAY2RGB)
                self.display_image(self.processed_preview, mask_rgb)
        else:
            self.toggle_preview_btn.setText("切换：显示带污渍图")
    ...
    def create_subfolders(self):
        ...
        # 定义子文件夹路径
        self.stained_dir = os.path.join(self.output_dir, "stained_images")
        self.mask_dir = os.path.join(self.output_dir, "masks")

        try:
            # 创建子文件夹（如果不存在）
            os.makedirs(self.stained_dir, exist_ok=True)
            os.makedirs(self.mask_dir, exist_ok=True)
            self.log_message(f"已创建子文件夹: {self.stained_dir}")
            self.log_message(f"已创建子文件夹: {self.mask_dir}")
            return True
        except Exception as e:
            self.log_message(f"创建子文件夹失败: {str(e)}")
            QMessageBox.critical(self, "错误", f"无法创建子文件夹: {str(e)}")
            return False
    ...
    def refresh_preview(self):
        ...
        # 生成带污渍图像和对应的mask
        basic_img, basic_mask = self.apply_basic_stains(self.original_image.copy())
        final_img, advanced_mask = self.apply_advanced_stains(basic_img)

        # 合并基础污渍和高级污渍的mask
        combined_mask = np.maximum(basic_mask, advanced_mask)
        self.processed_mask = self.get_binary_mask(combined_mask)
    ...
    def start_processing(self):
        ...
        self.processing_thread = StainProcessingThread(
            self.image_paths, self.stained_dir, self.mask_dir,
    ...
    def processing_finished(self, success):
        ...
        if success:
            QMessageBox.information(
                self, "完成",
                f"所有图像生成完成！\n带污渍图像保存至: {self.stained_dir}\n掩码图像保存至: {self.mask_dir}"
            )
...
class StainProcessingThread(QThread):
    ...
    def __init__(self, image_paths, stained_dir, mask_dir, basic_stain_func, advanced_stain_func, mask_func,
                 output_format):
        super().__init__()
        self.image_paths = image_paths
        self.stained_dir = stained_dir  # 带污渍图像保存目录
        self.mask_dir = mask_dir  # 掩码图像保存目录
    ...
    def run(self):
        ...
        for i, path in enumerate(self.image_paths):
            ...
            try:
                ...
                # 应用污渍算法（同时获取mask）
                basic_img, basic_mask = self.basic_stain_func(rgb_image.copy())
                final_img, advanced_mask = self.advanced_stain_func(basic_img)

                # 合并并处理mask
                combined_mask = np.maximum(basic_mask, advanced_mask)
                final_mask = self.mask_func(combined_mask)

                # 保存带污渍图像到stained_images子文件夹
                filename = Path(path).stem
                img_output_path = os.path.join(self.stained_dir, f"{filename}_stained.{self.output_format}")
                bgr_image = cv2.cvtColor(final_img, cv2.COLOR_RGB2BGR)
                cv2.imwrite(img_output_path, bgr_image)

                # 保存mask图像到masks子文件夹
                mask_output_path = os.path.join(self.mask_dir, f"{filename}_mask.{self.output_format}")
                ...
                self.log_updated.emit(f"生成成功: {os.path.basename(img_output_path)} 和 Mask")
...
if __name__ == "__main__":
    ...
    window.setWindowTitle("数据集生成（带自动文件夹分类）")
